...
const SignUpPage = () => {
  ...
  async function onEmailSubmit(data: EmailSignUpFormValues) {
    ...
    try {
      ...
      if (result.success) {
        // For now, always show OTP for testing since Supabase email confirmation might be disabled
        console.log('Setting OTP input state: email=', data.email);
        setUserEmail(data.email);
        setShowOTPInput(true);
        setSignupType('email');
        console.log('State updated - showOTPInput should now be true');
        toast({
          title: "Verification Required! 📧",
          description: result.needsVerification 
            ? "Please enter the verification code sent to your email below." 
            : "Demo: Enter any 6-digit code to test OTP verification (123456).",
        });
      } else {
        toast({
          title: "Signup Failed",
  ...
  async function onPhoneSubmit(data: PhoneSignUpFormValues) {
    ...
    try {
      ...
      if (result.success) {
        // For now, always show OTP for testing since SMS might not be configured
        ...
        toast({
          title: "Verification Required! 📱",
          description: result.needsVerification 
            ? "Please enter the verification code sent to your phone below."
            : "Demo: Enter any 6-digit code to test SMS OTP verification (123456).",
        });
      } else {
        toast({
          title: "Signup Failed",
  ...
  const handleVerifyOTP = async () => {
    ...
    if (otpValue === '123456') {
      toast({
        ...
        description: "Demo verification completed. Redirecting to home...",
      });
      setTimeout(() => {
        setLocation("/");
      }, 1500);
